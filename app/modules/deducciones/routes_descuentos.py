...
async def get_descuento_by_nro(nro_colegio: int, db: AsyncSession = Depends(get_db)):
    row = (
        await db.execute(select(Descuentos).where(Descuentos.nro_colegio == nro_colegio))
    ).scalars().first()
    if not row:
...
async def patch_descuento(desc_id: int, payload: DescuentoInPatch, db: AsyncSession = Depends(get_db)):
    row = await db.get(Descuentos, desc_id)
    if not row:

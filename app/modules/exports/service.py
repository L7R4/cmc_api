...
def build_excel_from_liquidacion(payload: Dict[str, Any]) -> bytes:
    ...
    ws1.cell(fila, 2, ", ".join(solicitud.get("obra_sociales", [])))
    fila += 1

    ws1.cell(fila, 1, "Períodos")
    ws1.cell(fila, 2, ", ".join(solicitud.get("periodos_normalizados", [])))
    fila += 1
    ...
    for k_print, k_key in [
        ...
        ws1.cell(fila, 2, resumen.get(k_key, 0))
        fila += 1

    _autosize(ws1)

    # Hoja Detalle por médico
